...
export class UserServices {
  ...
  /**
     * Creates a new user and returns the created instance (without password).
     * This method performs validation of relations and uniqueness.
     * Returns: { status: 'CREATED SUCCESSFULLY', user: <plain user object> }
     */
  async createOne(newUser, options = {}) {
    const t = options.transaction || null;
    try {
      // ensure the required role exists
      const userRole = await Role.findByPk(newUser.roleId, { transaction: t });
      if (!userRole) {
      ...
      if (newUser.genderId) {
        const userGender = await Gender.findByPk(newUser.genderId, { transaction: t });
        if (!userGender) throw Boom.notFound('Gender not found');
      }

      if (newUser.documentTypeId) {
        const userDocumentType = await DocumentType.findByPk(newUser.documentTypeId, { transaction: t });
        if (!userDocumentType) throw Boom.notFound('Document type not found');
      }

      // uniqueness checks
      const existingEmail = await User.findOne({ where: { email: newUser.email }, transaction: t });
      if (existingEmail) throw Boom.conflict('Email already registered');

      const existingUsername = await User.findOne({ where: { username: newUser.username }, transaction: t });
      if (existingUsername) throw Boom.conflict('Username already taken');

      if (newUser.documentNumber) {
        const existingDocument = await User.findOne({ where: { documentNumber: newUser.documentNumber }, transaction: t });
        if (existingDocument) throw Boom.conflict('Document number already registered');
      }
      ...
      // return created user (plain object) but remove password
      const createdPlain = created.get({ plain: true });
      delete createdPlain.password;

      return { status: 'CREATED SUCCESSFULLY', user: createdPlain };
  ...
  /**
 * Creates a new user and a business (negocio) for that user inside a DB transaction.
 * newUser: user data (as in createOne)
 * businessData: { name, description?, facebook?, instagram?, tiktok?, website?, locationId? }
 *
 * Returns: { status: 'CREATED SUCCESSFULLY', user: <plain>, business: <plain> }
 */
  async createOneWithBusiness({ newUserData, business }) {
  // Ensure models are available in this scope:
  // import { User } from '../models/user.js';
  // import { Business } from '../models/business.js';
  // import Boom from '@hapi/boom';

  // Use the sequelize instance attached to the User model for transactions
  const transaction = await User.sequelize.transaction();

  try {
    // Create user within transaction
    const createdUser = await User.create(newUserData, { transaction });

    // Prepare business payload and link to the created user.
    // IMPORTANT: change `userId` below to the actual FK field your Business model expects
    const businessPayload = {
      ...business,
      userId: createdUser.id, // <-- adjust this key if your FK is different (e.g. usuarioId, id_usuario)
    };

    // Create business within the same transaction
    const createdBusiness = await Business.create(businessPayload, { transaction });
    ...
    return {
      ...
      user: createdUser.get ? createdUser.get({ plain: true }) : createdUser,
      business: createdBusiness.get ? createdBusiness.get({ plain: true }) : createdBusiness,
    };
  } catch (error) {
    ...
    console.error('createOneWithBusiness ERROR:', error && (error.stack || error));

    // Handle common Sequelize errors with friendly responses (rethrow so caller/controller can respond)
    if (error && error.name === 'SequelizeUniqueConstraintError') {
      const fields = Object.keys(error.fields || {}).join(', ') || 'unknown';
      throw Boom.conflict(`Duplicate value for fields: ${fields}`, { details: error.errors?.map(e => e.message) });
    }

    if (error && error.name === 'SequelizeForeignKeyConstraintError') {
      throw Boom.badRequest('Foreign key constraint error (related resource not found)');
    }

    // If it's already a Boom error, rethrow it
    if (Boom.isBoom && Boom.isBoom(error)) throw error;

    // Wrap other errors
    throw Boom.boomify(error, { message: 'Unable to create new user with business' });
  }
}
  /**
